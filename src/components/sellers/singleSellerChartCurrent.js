...
class _Chart extends React.Component {
  ...
  componentWillMount() {
        // debugger;
    scrollTo(0, 0);
    const id = this.props.params.id;  // 根据路由获取该商店id
  ...
  componentDidMount() {
    	...
    	const domLine = ReactDOM.findDOMNode(this.refs.singleSellerCustomerNumChart);
  ...
  componentWillReceiveProps(nextProps, nextState) {
    ...
    if (customerNum.series[0] && customerNum.series[0].data && customerNum.series[0].data[0]) {
      const now = {numLen: '', num: '', doorNumLen: '', doorNum: '', percentLen: '', percent: ''};
      const yesterday = customerNum.xAxis[0].yesterday;
            // 四个分别是时间、客流量、门前客流、客流占比
      const timeList = customerNum.xAxis[0].data.reverse();
      const numList = customerNum.series[0].data.reverse();
      const doorNumList = customerNum.series[1].data.reverse();
      const percentList = customerNum.series[2].data.reverse();
  ...
  render() {
    const {now, yesterday, timeList, numList, doorNumList, percentList, tableSpace} = this.state;
    const rows = [];
    if (timeList) {
      ...
      <div className="panel">
        ...
        <div className="panelBody">
          <div className="singleSellerCustomerNumChart" ref="singleSellerCustomerNumChart" />
        </div>
      </div>
      <div className="panel">
...
export default connect(mapStateToProps, sellersAction)(_Chart);
