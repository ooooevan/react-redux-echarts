...
class _Chart extends React.Component {
  ...
  componentDidMount() {
    const domLine = ReactDOM.findDOMNode(this.refs.chartLine);
  ...
  componentWillReceiveProps(nextProps, nextState) {
    const line = nextProps.line.toJS();
    if (line.xAxis.data) {
  ...
  render() {
    const rows = [];
    const {yesterday, chartNum, chartTime, timeList, dataList, tableSpace} = this.state;
    if (timeList) {
