...
class _sellersAvg extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.props.sellersAndTime.length > 1) {
      ...
      const dom = ReactDOM.findDOMNode(this.refs.compareSellerAvgChart);
  ...
  componentWillReceiveProps(nextProps, nextState) {
    ...
    const sellersAvg = nextProps.sellersAvg.toJS();
    if (sellersAvg.series[0].data && sellersAvg.series[0].data[0]) {
      const timeList = sellersAvg.xAxis[0].data;
      const num1List = sellersAvg.series[0].data;
      const num2List = sellersAvg.series[1].data;
      const percent1List = sellersAvg.xAxis[0].percent1;
      const percent2List = sellersAvg.xAxis[0].percent2;
    ...
    return (<div>
      <div className="panel">
        ...
        <div className="panelBody">
          <div ref="compareSellerAvgChart" className="compareSellerAvgChart" />
        </div>
      </div>
      <div className="panel">
...
export default connect(mapStateToProps, compareAction)(_sellersAvg);
