...
class _timeSection extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.props.sellersAndTime.length > 1) {
      ...
      const dom = ReactDOM.findDOMNode(this.refs.compareSellerTimeSectionChart);
      this.state.compareSellerTimeSectionChart = echarts.init(dom);
      this.state.compareSellerTimeSectionChart.showLoading();
    }
 ...
 //
  ...
  componentWillReceiveProps(nextProps, nextState) {
    ...
    if (timeSection.series[0].data && timeSection.series[0].data && timeSection.series[0].data[0]) {
      ...
      const num1List = timeSection.series[0].data;
      const num2List = timeSection.series[1].data;
    ...
    return (<div>
      <div className="panel">
        ...
        <div className="panelBody">
          <div ref="compareSellerTimeSectionChart" className="compareSellerTimeSectionChart" />
        </div>
      </div>
      <div className="panel">
...
export default connect(mapStateToProps, compareAction)(_timeSection);
