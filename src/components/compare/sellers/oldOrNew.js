...
class _sellersOldOrNew extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.props.sellersAndTime.length > 1) {
      ...
      const dom = ReactDOM.findDOMNode(this.refs.compareSellerOldOrNewChart);
      this.state.compareSellerOldOrNewChart = echarts.init(dom);
      this.state.compareSellerOldOrNewChart.showLoading();
    }
 ...
 //
  ...
  componentWillReceiveProps(nextProps, nextState) {
    ...
    if (OldOrNew.series[0].data && OldOrNew.series[0].data && OldOrNew.series[0].data[0]) {
      ...
      const percent1List = OldOrNew.series[0].data;
      const percent2List = OldOrNew.series[1].data;
      const newNum1List = OldOrNew.xAxis[0].newNum1;
      const newNum2List = OldOrNew.xAxis[0].newNum2;
      const oldNum1List = OldOrNew.xAxis[0].oldNum1;
      const oldNum2List = OldOrNew.xAxis[0].oldNum2;
 ...
 //
  ...
  render() {
    	const {timeList, newNum1List, oldNum1List, oldNum2List, newNum2List, percent1List, percent2List, seller1, seller2} = this.state;
    const rows = [];
    if (timeList) {
    ...
    return (<div>
      <div className="panel">
        ...
        <div className="panelBody">
          <div ref="compareSellerOldOrNewChart" className="compareSellerOldOrNewChart" />
        </div>
      </div>
      <div className="panel">
...
export default connect(mapStateToProps, compareAction)(_sellersOldOrNew);
