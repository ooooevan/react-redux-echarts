...
class _cycle extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.props.sellersAndTime.length > 1) {
      ...
      const dom = ReactDOM.findDOMNode(this.refs.compareSellerCycleChart);
      this.state.compareSellerCycleChart = echarts.init(dom);
      this.state.compareSellerCycleChart.showLoading();
    }
 ...
 //
  ...
  render() {
    ...
    return (<div>
      <div className="panel">
        <div className="panelHead">顾客来访周期对比&nbsp;<FaQuestion className="questionMark" />
          <div className="messageMark"><p>展示两个商家在同一时间段内的来访周期对比<br /></p></div></div>
        <div className="panelBody">
          <div ref="compareSellerCycleChart" className="compareSellerCycleChart" />
        </div>
      </div>
      <div className="panel">
        <div className="panelHead">顾客来访周期对比明细</div>
        <div className="panelBody">
