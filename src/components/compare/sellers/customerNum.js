...
class _sellersNum extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.props.sellersAndTime.length > 1) {
      ...
      const dom = ReactDOM.findDOMNode(this.refs.compareSellerNumChart);
      this.state.compareSellerNumChart = echarts.init(dom);
      this.state.compareSellerNumChart.showLoading();
    }
  ...
  componentWillReceiveProps(nextProps, nextState) {
    ...
    const sellersNum = nextProps.sellersNum.toJS();
    if (sellersNum.series[0].data && sellersNum.series[0].data[0]) {
      const timeList = sellersNum.xAxis[0].data;
      const num1List = sellersNum.series[0].data;
      const num2List = sellersNum.series[1].data;
      const percent1List = sellersNum.xAxis[0].percent1;
      const percent2List = sellersNum.xAxis[0].percent2;
    ...
    return (<div>
      <div className="panel">
        <div className="panelHead">客流量峰值对比&nbsp;<FaQuestion className="questionMark" />
          <div className="messageMark"><p>展示两个商家在同一时间段内的客流峰值对比<br /></p></div></div>
        <div className="panelBody">
          <div ref="compareSellerNumChart" className="compareSellerNumChart" />
        </div>
      </div>
      <div className="panel">
...
export default connect(mapStateToProps, compareAction)(_sellersNum);
