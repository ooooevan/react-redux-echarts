...
class _customerAvg extends React.Component {
 ...
 //        this.props.comparenumInit(this.state.time,this.state.chartPage);
  ...
  componentDidMount() {
    ...
    const dom = ReactDOM.findDOMNode(this.refs.compareCustomerAvgChart);
 ...
 //
  ...
  componentWillReceiveProps(nextProps, nextState) {
    if (this.props !== nextProps) {  // 当props改变时才触发。只改变selectTime不触发
      const customerAvg = nextProps.customerAvg.toJS();
      const timeList = customerAvg.xAxis[0].data;
      const num1List = customerAvg.series[0].data;
      const num2List = customerAvg.series[1].data;
    ...
    return (<div className="chartWrapper">
      ...
      <div className="panel">
        ...
        <div className="panelBody">
          <div ref="compareCustomerAvgChart" className="compareCustomerAvgChart" />
        </div>
      </div>
      <div className="panel">
...
export default connect(mapStateToProps, compareAction)(_customerAvg);
